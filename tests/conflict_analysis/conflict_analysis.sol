...
contract ConflictTest{
    ...
    function simpleMapReadWithEnvKey(uint x) public returns (uint256) {
        if(x == 1){
    ...
    // No storage access
    function noStorageAccessAndContractCalling(uint x) public returns (uint) {
        return x; //5, NoConflict
    }
    function noStorageAccessHasContractCalling(address addr) public returns (uint) {
